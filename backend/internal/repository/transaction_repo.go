...
// Ensure TransactionRepo satisfies the TransactionStore interface at compile time.
var _ TransactionStore = (*TransactionRepo)(nil)
...
func (r *TransactionRepo) Create(ctx context.Context, t *domain.Transaction) (*domain.Transaction, error) {
	...
	).Scan(&out.ID, &out.UserID, &out.Amount, &out.Category,
		&out.Description, &out.Date, &out.CreatedAt)
...
func (r *TransactionRepo) ListByUser(ctx context.Context, userID string, limit, offset int) ([]*domain.Transaction, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.UserID, &t.Amount, &t.Category,
...
// Called after every transaction insert to check budgets.
