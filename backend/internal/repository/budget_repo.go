...
// Ensure BudgetRepo satisfies the BudgetStore interface at compile time.
var _ BudgetStore = (*BudgetRepo)(nil)
...
func (r *BudgetRepo) Upsert(ctx context.Context, b *domain.Budget) (*domain.Budget, error) {
	...
	).Scan(&out.ID, &out.UserID, &out.Category, &out.LimitAmount,
		&out.Month, &out.CreatedAt)
...
func (r *BudgetRepo) ListByUserMonth(ctx context.Context, userID string, month time.Time) ([]*domain.Budget, error) {
	...
	for rows.Next() {
		var b domain.Budget
		if err := rows.Scan(&b.ID, &b.UserID, &b.Category,
			&b.LimitAmount, &b.Month, &b.CreatedAt); err != nil {
			return nil, err
		}
